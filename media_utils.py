...
def download_youtube_video(
    ...
    # Try to extract video ID for better filename generation
    video_id = None
    ...
    # Use video ID if available, otherwise use the hash
    if video_id:
        output_template = os.path.join(output_dir, f"youtube_{video_id}_%(title)s.%(ext)s")
    else:
        output_template = os.path.join(output_dir, f"youtube_{url_hash}_%(title)s.%(ext)s")
    ...
    # Define different format selection strategies to try
    format_strategies = [
        f"bestvideo[height<={max_height}]+bestaudio/best[height<={max_height}]/best",  # Best video up to max height + best audio
        f"bestvideo+bestaudio/best",  # Any best quality combination
        "best",  # Single best format
    ]

    for tool in yt_tools:
        if downloaded_path:
            break  # Skip if we already succeeded with a previous tool
            
        # Try the regular format strategies first
        for format_strategy in format_strategies:
            if downloaded_path:
                break  # Skip if we already succeeded with a previous format
                
            try:
                logger.info(f"Attempting download of '{url}' using {tool} with format strategy: {format_strategy}")
                ...
                cmd = [
                    ...
                    "--verbose",                    # Output more info for debugging
                    "--no-check-certificate",       # Skip HTTPS certificate validation
                    "--geo-bypass",                 # Try to bypass geo-restrictions
                    "--no-cache-dir",               # Don't use download cache
                    url,                            # The video URL
                ]
                ...
                logger.debug(f"Process stdout (first 300 chars): {process.stdout[:300]}")
                logger.debug(f"Process stderr (first 300 chars): {process.stderr[:300]}")

                if process.returncode == 0:
                    # Success! Now we need to find the actual output file path.
                    # Display directory contents for debugging
                    logger.info(f"Download completed with {tool}. Finding output file...")
                    logger.debug(f"Files in {output_dir}: {os.listdir(output_dir)}")
                    
                    # Try different methods to find the file:
                    
                    # 1. Direct filename search using video ID or hash
                    search_patterns = []
                    if video_id:
                        search_patterns.append(f"youtube_{video_id}")
                    search_patterns.append(f"youtube_{url_hash}")
                    
                    # Find the most recently created/modified file in the directory that matches our patterns
                    most_recent_file = None
                    most_recent_time = 0
                    
                    for filename in os.listdir(output_dir):
                        file_path = os.path.join(output_dir, filename)
                        if os.path.isfile(file_path) and any(pattern in filename for pattern in search_patterns):
                    ...
                    if most_recent_file:
                        downloaded_path = most_recent_file
                        logger.info(f"Found downloaded video file: {downloaded_path}")
                        break
                    
                    # 2. If still not found, look for any video file created in the last minute
                    if not downloaded_path:
                        now = time.time()
                        for filename in os.listdir(output_dir):
                            file_path = os.path.join(output_dir, filename)
                            # Check if it's a file and was created within the last minute
                            if (os.path.isfile(file_path) and 
                                os.path.getmtime(file_path) > now - 60 and
                                filename.endswith((".mp4", ".mkv", ".webm"))):
                                downloaded_path = file_path
                                logger.info(f"Found recently created video file: {downloaded_path}")
                                break
                    
                    # 3. Look for output filename in stdout (yt-dlp often reports the file it created)
                    if not downloaded_path:
                        stdout_lines = process.stdout.split('\n')
                        for line in stdout_lines:
                            if '[download]' in line and 'Destination' in line and '.mp4' in line:
                                try:
                                    path_match = line.split('Destination: ')[1].strip()
                                    potential_path = os.path.join(output_dir, os.path.basename(path_match))
                                    if os.path.exists(potential_path):
                                        downloaded_path = potential_path
                                        logger.info(f"Found video file from stdout: {downloaded_path}")
                                        break
                    ...
                    if not downloaded_path:
                        ...
                        logger.error(f"{tool} reported success, but couldn't find the output file in {output_dir}.")
                        logger.debug(f"Full stdout: {process.stdout}")
                        logger.debug(f"Full stderr: {process.stderr}")
                    ...
                    if "This video is not available" in error_msg:
                        logger.error(f"Video unavailable error: {error_msg}")
                    elif "copyright grounds" in error_msg or "copyright claim" in error_msg:
                        logger.error(f"Copyright restriction error: {error_msg}")
                    elif "account has been terminated" in error_msg:
                        logger.error(f"Terminated account error: {error_msg}")
                    elif "not available in your country" in error_msg:
                        logger.error(f"Geographic restriction error: {error_msg}")
                        # Try a different format if geo-restricted
                        continue
                    else:
                        logger.warning(
                            f"{tool} download failed with format {format_strategy} (exit code {process.returncode})."
                            f"\nStderr: {error_msg[:300]}"
                        )
                    
                    # Continue to the next format strategy or tool

            except FileNotFoundError:
                logger.warning(f"Download tool '{tool}' not found in PATH. Trying next...")
                if tool == "yt-dlp" and "youtube-dl" not in yt_tools:
                    # If yt-dlp is not found and youtube-dl is also not in the list, show helpful message
                    logger.error(
                        "Neither yt-dlp nor youtube-dl found. Please install one of these tools to download from YouTube."
                        "\nTo install yt-dlp: 'pip install yt-dlp' or 'pip install -U yt-dlp'"
                    )
        ...
        # If all format strategies failed, try one last simple approach with this tool
        if not downloaded_path:
            try:
                logger.info(f"Attempting final simplified download approach for '{url}' with {tool}")
                # Simplest possible command with minimal options
                simple_cmd = [
                    tool,
                    "--no-playlist",
                    "--format", "best",  # Just get the best available format
                    "--output", os.path.join(output_dir, f"final_attempt_{url_hash}.%(ext)s"),
                    url
                ]
                
                logger.info(f"Running simplified command: {' '.join(simple_cmd)}")
                process = subprocess.run(simple_cmd, capture_output=True, text=True, check=False)
                
                if process.returncode == 0:
                    # Look for any newly created file
                    newest_file = None
                    newest_time = 0
                    
                    for filename in os.listdir(output_dir):
                        if "final_attempt" in filename:
                            file_path = os.path.join(output_dir, filename)
                    ...
                    if newest_file:
                        downloaded_path = newest_file
                        logger.info(f"Successfully downloaded with simplified approach: {downloaded_path}")
            except Exception as e:
                logger.error(f"Final simplified download attempt failed with {tool}: {e}")
                # Continue to next tool

    if not downloaded_path:
        # Detailed error message
        logger.error(f"All download attempts failed for URL: {url}")
        logger.error("Possible reasons: video is private, removed, region-restricted, or requires login")
    ...
    if not os.path.exists(downloaded_path) or os.path.getsize(downloaded_path) < 10000:  # Less than 10KB is probably not a valid video
        logger.error(f"Downloaded file is missing or too small: {downloaded_path}")
        return None

    logger.info(f"Successfully downloaded video to: {downloaded_path} ({os.path.getsize(downloaded_path)/1024/1024:.2f} MB)")
