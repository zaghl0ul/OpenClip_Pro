...
def _display_project_thumbnail(thumbnail_path, width=100, use_container_width=False):
    """Displays a project thumbnail image if available."""
    if thumbnail_path and os.path.exists(thumbnail_path):
        try:
            st.image(thumbnail_path, width=width if not use_container_width else None, use_container_width=use_container_width)
        except Exception as e:
...
def _display_list_view(projects):
    ...
    for project in projects:
        ...
        with item_cols[0]:
            _display_project_thumbnail(top_clip_thumbnail, use_container_width=False, width=100)
    ...
    for project in projects:
        ...
        with cols[col_index % columns]:
            ...
            with st.container(border=True):
                st.markdown(f"**{pname}**")
                _display_project_thumbnail(top_clip_thumbnail, use_container_width=True)
