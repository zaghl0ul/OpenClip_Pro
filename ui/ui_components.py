...
def create_api_key_management_ui(
    ...
    st.markdown(
        """
    Configure API keys for third-party AI services. These keys are required for
    certain AI models but remain on your device (they're stored encrypted).
    """
    )

    # Check for insecure key usage and display warning
    if os.environ.get("OPENCLIP_FERNET_KEY") is None:
        st.warning(
            "⚠️ Using default encryption key for API storage. This is less secure than using your own key. "
            "For better security, set the OPENCLIP_FERNET_KEY environment variable.",
            icon="⚠️"
        )
